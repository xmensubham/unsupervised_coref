...
\noindent The project work has been prepared as per the regulations of \textbf{Central Institute of Technology} and qualifies to be accepted in partial fulfillment of the requirement for the degree of B.Tech in \textbf{Computer Science and Engineering}.\\[0.7cm]
...
Natural Language Processing (NLP) has witnessed transformative progress over the past decade, largely driven by the emergence of large-scale pretrained transformer models such as BERT \cite{devlin2018bert}, RoBERTa \cite{liu2019roberta}, and their multilingual and language-specific descendants. However, the overwhelming majority of these advances have concentrated on English and a small cluster of high-resource European languages. For Hindi --- a language with over 600 million native and second-language speakers, making it the third most spoken language on earth --- the NLP ecosystem remains comparatively sparse, particularly for complex discourse-level tasks such as \textbf{coreference resolution}.

\textbf{Coreference resolution} is the task of identifying all expressions in a text that refer to the same real-world entity and grouping them into \textit{coreference chains} (also called entity clusters). Consider the following example in Hindi:
...
A coreference resolver must identify that \textit{Ram} and \textit{Vah} (he) refer to the same person and place them in the same cluster. This seemingly simple inference requires the system to understand pronouns, nominal phrases, discourse structure, and the morphological properties of a highly inflectional language. It is a prerequisite for high-quality machine translation, question answering, dialogue systems, information extraction, and text summarisation.

\section{The Challenge for Hindi}

Hindi poses a distinctive set of challenges that make coreference resolution substantially harder than in English:

\begin{itemize}
    \item \textbf{Morphological richness:} Hindi nouns and verbs are heavily inflected for gender, number, case, and tense. The same pronoun (\textit{vah}) can refer to a person of any gender, requiring context-sensitive disambiguation.
    \item \textbf{Pro-drop:} Hindi frequently omits subject pronouns, forcing the system to infer implicit arguments.
    \item \textbf{Free word order:} Subject-Object-Verb (SOV) is canonical, but deviations are common, complicating mention detection and antecedent ranking.
    \item \textbf{Devanagari script complexity:} Conjuncts, matras, and zero-width joiners make tokenisation non-trivial.
    \item \textbf{Scarcity of annotated data:} Unlike English, which has the OntoNotes 5.0 corpus (tens of thousands of coreference-annotated documents), Hindi has extremely limited publicly available coreference datasets.
\end{itemize}

\section{Project Journey Overview}

This project was carried out in two major phases, and the honest account of both --- including a significant methodological failure --- constitutes an important part of the research contribution.

\subsection{Phase 1: Scratch BERT Training (Exploration and Failure)}

The initial approach was to train a custom BERT-style transformer model from scratch on a large Hindi corpus (approximately 172,000 Hindi Wikipedia documents sourced from Kaggle). A SentencePiece BPE tokenizer with a vocabulary size of 32,000 was trained on the same data. The model was trained using the Masked Language Modeling (MLM) objective, and the resulting contextual embeddings were used for unsupervised semantic clustering of mentions.

While the embeddings showed reasonable semantic coherence --- semantically related words were grouped together --- the approach \textbf{failed fundamentally} when applied to coreference resolution. Pronouns such as \textit{vah}, \textit{usne}, and \textit{vahaan} were not correctly linked to their antecedents. Clusters were inconsistent, and the similarity thresholds that governed clustering had no principled basis. The core lesson was clear: \textbf{coreference resolution is not a semantic similarity problem}. It requires explicit mention detection, antecedent ranking, syntactic awareness, and discourse modelling --- none of which pure embedding clustering provides.

\subsection{Phase 2: Hybrid Transformer Architecture (Successful Pipeline)}

Following this failure, the project pivoted to a supervised hybrid architecture. The key design decisions were:

\begin{enumerate}
    \item Adopt \textbf{IndicBERTv2 (MLM-only)} from AI4Bharat as the contextual embedding backbone, leveraging its existing Hindi linguistic knowledge instead of training from scratch.
    \item \textbf{Manually create an annotated coreference dataset} of 76 documents and 548 mention annotations, since no suitable Hindi dataset was available.
    \item Formulate coreference as a \textbf{pairwise mention classification} problem: for each pair of mentions, predict whether they are coreferent.
    \item Combine IndicBERT embeddings with \textbf{hand-crafted Hindi linguistic features} (pronoun detection, plurality, clause heuristics, mention compatibility) as input to a \textbf{Logistic Regression} classifier.
    \item Reconstruct entity clusters from predicted coreferent pairs using a \textbf{Union-Find} algorithm.
\end{enumerate}
...
\begin{itemize}
    \item A documented exploration and principled failure analysis of the scratch-BERT unsupervised clustering approach for Hindi coreference.
    \item A manually annotated Hindi coreference dataset of 76 documents and 548 mention spans.
    \item A working end-to-end Hindi coreference pipeline combining IndicBERTv2 embeddings, Hindi-specific linguistic features, logistic regression pairwise classification, and Union-Find cluster reconstruction.
    \item An honest empirical evaluation of the system including pairwise classification metrics (precision, recall, F1) and MUC coreference scoring.
    \item Identification of key bottlenecks and a concrete roadmap for future improvement.
\end{itemize}

\section{Report Organisation}

The remainder of this report is organised as follows. Chapter~2 presents the motivation and objectives. Chapter~3 surveys related work. Chapter~4 details the proposed methodology. Chapter~5 presents experimental results. Chapter~6 concludes with a discussion of drawbacks. Chapter~7 outlines future directions.
...
Hindi is the official language of India, spoken natively by over 530 million people and understood by an additional 100 million as a second language. Despite this scale, Hindi remains a \textbf{low-resource language} in the NLP sense: large-scale annotated corpora for tasks such as coreference resolution, semantic role labelling, and discourse parsing are either absent or extremely limited in size and quality. This stands in sharp contrast to English, which benefits from benchmarks such as OntoNotes 5.0 (containing thousands of coreference-annotated documents). The direct consequence is that downstream NLP applications for Hindi --- machine translation, virtual assistants, summarisation, question answering, information extraction --- either perform poorly or simply do not exist at a production level.

\subsection{Coreference Resolution as a Core NLP Capability}

Coreference resolution is not an isolated research curiosity; it is a fundamental capability that underlies many practical NLP systems:

\begin{itemize}
    \item \textbf{Machine Translation:} Correct pronoun translation requires knowing the gender and number of the antecedent, which in Hindi is embedded in the coreference structure.
    \item \textbf{Question Answering:} Answering ``Who went to school?'' after reading a multi-sentence passage requires linking the pronoun to its referent.
    \item \textbf{Document Summarisation:} Coherent summaries must track entities across sentences, avoiding dangling pronouns.
    \item \textbf{Dialogue Systems:} Understanding ``Tell me more about him'' requires resolving \textit{him} to a previously mentioned entity.
    \item \textbf{Information Extraction:} Building knowledge graphs from Hindi text requires entity consolidation across mentions.
\end{itemize}

Without coreference resolution, all of these applications face a hard ceiling on performance.

\subsection{Failure of Generic Approaches for Hindi}

Generic multilingual models such as mBERT and XLM-RoBERTa, while impressive for many tasks, are not optimally suited to Hindi coreference because:
\begin{itemize}
    \item They treat all languages with equal weight, diluting Hindi-specific morphological patterns.
...
\begin{enumerate}
    ...
    \item \textbf{Create a Hindi coreference dataset:} Manually annotate a corpus of 76 Hindi documents with 548 coreference mention spans, cluster IDs, and mention types, addressing the resource scarcity problem.
...
\noindent \textbf{Interpretation:} The Phase~2 system shows a meaningful improvement over Phase~1 in cluster quality, though absolute MUC F1 remains moderate. This is expected given the small dataset size (76 documents), the absence of automatic mention detection, and the ceiling imposed by logistic regression on high-dimensional embedding features. B$^3$ and CEAF metrics were not computed (see limitations in Chapter~6).
...
\begin{itemize}
    \item Cluster 1: \{Ram, Vah, Usne\}
\end{itemize}

\textbf{System predicted clusters:}
\begin{itemize}
    \item Cluster 1 (correct): \{Ram, Vah, Usne\} --- pronoun flags and clause heuristics correctly triggered
\end{itemize}
...
\textbf{System predicted:} \{Seema, Vah\} --- the system incorrectly resolved the pronoun to the first-mentioned entity rather than recognising the ambiguity or the plural-to-singular mismatch.
...
\begin{itemize}
    ...
    \item \textbf{Manually annotated dataset} of 76 Hindi documents with 548 coreference mention spans --- a genuine low-resource NLP contribution.
...
\begin{enumerate}
    \item \textbf{Small size:} 76 documents and 548 mention spans is an extremely small dataset for a supervised machine learning task. The model's generalisation beyond the annotated domain is uncertain.
...
\begin{enumerate}
    ...
    \item \textbf{Small test set:} Evaluation on held-out folds of a 76-document dataset yields high-variance estimates of true system performance.
...
\begin{itemize}
    \item \textbf{Scale annotation:} Expand the dataset from 76 to at least 300--500 documents through continued manual annotation or crowdsourcing with quality control.
